...
| <a name="input_security_group_config"></a> [security\_group\_config](#input\_security\_group\_config) | Configuration for security group, leave empty to not create a security group. When this is set the subnet\_ids also needs to be set | <pre>object({<br>    name_prefix = optional(string, null)<br>    egress_rules = optional(list(object({<br>      cidr_ipv4                    = optional(string)<br>      cidr_ipv6                    = optional(string)<br>      description                  = string<br>      from_port                    = optional(number)<br>      ip_protocol                  = optional(string, "-1")<br>      prefix_list_id               = optional(string)<br>      referenced_security_group_id = optional(string)<br>      to_port                      = optional(number)<br>    })), [])<br>    ids = optional(list(string), [])<br>  })</pre> | <pre>{<br>  "egress_rules": [],<br>  "ids": [],<br>  "name_prefix": null<br>}</pre> | no |
